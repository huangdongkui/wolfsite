# wolfsite 企业信息化快速开发平台

## 平台简介

wolfsite是基于多个优秀的开源项目，高度整合封装而成的高效，高性能，强安全性的**开源**Java EE快速开发平台。

wolfsite是您快速完成项目的最佳基础平台解决方案，wolfsite是您想学习Java平台的最佳学习案例，wolfsite还是接私活的最佳助手。

wolfsite是在Spring Framework基础上搭建的一个Java基础开发平台，以Spring MVC为模型视图控制器，MyBatis为数据访问层，
Apache Shiro为权限授权层，Ehcahe对常用数据进行缓存，Activit为工作流引擎。是JavaEE界的最佳整合。

wolfsite主要定位于企业信息化领域，已内置企业信息化系统的基础功能和高效的**代码生成**工具，
包括：系统权限组件、数据权限组件、数据字典组件、核心工具组件、视图操作组件、工作流组件、代码生成等。
前端界面风格采用了结构简单、性能优良、页面美观大气的Twitter Bootstrap页面展示框架。
采用分层设计、双重验证、提交数据安全编码、密码加密、访问验证、数据权限验证。
使用Maven做项目管理，提高项目的易开发性、扩展性。

wolfsite目前包括以下四大模块，系统管理（SYS）模块、
...
wolfsite 提供了常用工具进行封装，包括日志工具、缓存工具、服务器端验证、数据字典、当前组织机构数据
（用户、机构、区域）以及其它常用小工具等。另外还提供一个强大的在线 **代码生成** 工具，
此工具提供简单的单表、一对多、树结构功能的生成，如果对外观要求不是很高，生成的功能就可以用了。
如果你使用了wolfsite基础框架，就可以很高效的快速开发出，优秀的信息管理系统。
...
* <http://demo.wolfsite.com/wolfsite>  &nbsp; 用户名：fanhua &nbsp; 密码：admin

## 快速体验

1. 具备运行环境：JDK1.6+、Maven3.0+、MySql5+或Oracle10g+。
2. 修改src\main\resources\wolfsite.properties文件中的数据库设置参数。
...
6. 最高管理员账号，用户名：fanhua 密码：admin

## 常见问题

1. 用一段时间提示内存溢出，请修改JVM参数：-Xmx512m -XX:MaxPermSize=256m
2. 有时出现文字乱码：修改Tomcat的server.xml文件的Connector项，增加URIEncoding="UTF-8"
3. 为什么新建菜单后看不到新建的菜单？因为授权问题，菜单管理只允许最高管理员账号管理（最高管理员默认账号：fanhua 密码：admin）。

## 更多文档

* <https://github.com/fanhua/wolfsite/tree/master/doc>

## 如何交流、反馈、参与贡献？

* QQ Group：127515876 &nbsp; 209330483 &nbsp; 223507718 &nbsp; 苹果版QQ若不能加入请使用手机QQ最新版
* E-mail：fanhua@163.com
* GitHub：<https://github.com/fanhua/wolfsite>
* 开源中国：<http://git.oschina.net/fanhua/wolfsite>
* 官方网址：<http://wolfsite.com>  论坛：<http://bbs.wolfsite.com>
* 支持wolfsite发展：（加我好友）支付宝：fanhua@163.com &nbsp; 微信：fanhua
...
6. 你可以二次包装出售，**但还请保留文件中的版权和作者信息**，并在你的产品说明中注明wolfsite。
...
* 多数据库支持：有些人说MyBatis对多数据库支持困难，我认为这个不是问题，虽说目前wolfsite仅提供对MySql或Oracle
数据库的支持，但对于支持其它数据库的改动也不是很麻烦，SQL是被专门写在XML中，对于大多数SQL来说都是通用的，
对于不同的数据库可通过dbName区分和修改各别的SQL片段即可。

上述纯属个人观点，欢迎指正（fanhua@163.com）。

如果你仍然喜欢Hibernate，请点击[下载wolfsite Hibernate版本](https://github.com/fanhua/wolfsite/tree/master_hibernate)。
