 ...
 */
package com.fanhua.wolfsite.modules.act.web;
...
public class ActTaskController extends BaseController {
	...
	public String myApplyList(Act act, HttpServletRequest request, HttpServletResponse response, Model model){
		Page<Act> page = new Page<Act>(request, response);
		page = actTaskService.myApplyList(page, act);
		model.addAttribute("page", page);
		if (UserUtils.getPrincipal().isMobileLogin()){
			return renderString(response, page);
		}
		
		return "modules/act/actTaskMyApplyList";
	}
	/**
	 * 获取流转历史列表
	...
	public String form(Act act, HttpServletRequest request, Model model){
		...
		//String bid= actTaskService.getfHistProcIns((act.getProcInsId())).getBusinessKey().split(":")[1];
		//act.setBusinessId(bid);
...
//		return "modules/act/actTaskForm";
	 ...
	 * @param businessTableName 业务表名称
	 * @param taskTitle 任务标题
	 * @return
	 */
	@RequestMapping(value = "startFlow")
	@ResponseBody
	public String startFlow(String procDefKey,String businessTable,String businessId,String title){
		try {
			actTaskService.startProcess(procDefKey,businessTable, businessId, title);
			return "true";
		} catch (Exception e) {
	 ...
	 * 		vars.keys=flag,pass
	 * 		vars.values=1,true
	 * 		vars.types=S,B  @see com.fanhua.wolfsite.modules.act.utils.PropertyType
	 */
